...
fn forall_pred(array: &[i32], len: usize) -> (result: i32)
    ...
    ensures
        (result) as int != 0 <==> ((forall|i: int|
            (0 <= i < len) as int != 0 ==> (0 <= array@[(i) as int] <= 42) as int != 0)) as int
            != 0,
{
    let mut i: usize = 0;
    while i < len
        invariant
            0 <= i <= len,
            forall|j: int| (0 <= j < i) as int != 0 ==> (0 <= array@[(j) as int] <= 42) as int != 0,
        decreases len - i,
    {
        if pred(array[i]) == 0 {
            return 0;
...
fn exists_pred(array: &[i32], len: usize) -> (result: i32)
    ...
    ensures
        (result) as int != 0 <==> ((exists|i: int|
            0 <= i < len && 0 <= array@[(i) as int] <= 42)) as int != 0,
{
    let mut i: usize = 0;
    while i < len
        invariant
            0 <= i <= len,
            forall|j: int|
                (0 <= j < i) as int != 0 ==> ((!(((0 <= array@[(j) as int] <= 42)) as int
                    != 0))) as int != 0,
        decreases len - i,
    {
        if pred(array[i]) != 0 {
...
fn none_pred(array: &[i32], len: usize) -> (result: i32)
    ...
    ensures
        (result) as int != 0 <==> ((forall|i: int|
            (0 <= i < len) as int != 0 ==> ((!(((0 <= array@[(i) as int] <= 42)) as int
                != 0))) as int != 0)) as int != 0,
{
...
fn some_not_pred(array: &[i32], len: usize) -> (result: i32)
    ...
    ensures
        (result) as int != 0 <==> ((exists|i: int|
            0 <= i < len && (!(((0 <= array@[(i) as int] <= 42)) as int != 0)))) as int != 0,
{
